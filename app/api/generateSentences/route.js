...
export async function POST(req) {
  try {
    ...
    if (!word || typeof word !== 'string' || !/^[a-zA-Z]+$/.test(word)) {
      return new Response(
        JSON.stringify({ error: 'Invalid word. Please provide a valid alphabetic word.' }),
    ...
    const prompt = `
      Provide a definition of the word "${word}".
      Then generate between 3 and 5 sentences that use the word "${word}" in context.
    `;
    ...
    if (sentences.length < 3) {
      return new Response(
        JSON.stringify({ error: 'Could not generate enough sentences. Try again with a different word.' }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }

    // Return definition and sentences
    return new Response(JSON.stringify({ definitionAndSentences: completion }), {
      status: 200,
