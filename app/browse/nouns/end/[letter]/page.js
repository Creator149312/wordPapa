...
export async function generateMetadata({ params }) {
  ...
  return { 
    ...
    alternates: {
      canonical: `https://www.wordpapa.com/browse/nouns/end/${L}`,
    },
    robots: {
      index: toIndex,
      follow: true,
      ...
      <div className="px-4 md:px-8 py-8">
        <div className="p-8 bg-gray-50 dark:bg-white/5 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-[2rem] text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium">
            No nouns ending with <strong className="text-gray-700 dark:text-white">{L}</strong> were found. Try a different letter.
          </p>
        </div>
      </div>
    );
    ...
    <div className="px-4 md:px-6 py-8 md:py-12 space-y-8">
      <div>
        ...
        <p className="mt-4 text-lg text-gray-500 dark:text-gray-400 font-medium max-w-2xl">
          ...
          singular and plural nouns ending with {phraseSearch}{" "}
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 font-medium leading-relaxed max-w-3xl">
        All nouns ending with <strong className="text-gray-700 dark:text-white">{L.toUpperCase()}</strong> are sorted by length for easy browsing. Includes common nouns, proper nouns, concrete nouns, and abstract nouns.
      </p>
    </div>
  );
};
