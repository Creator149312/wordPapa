...
export default function GameHeader({
  ...
  const handleQuitClick = () => {
    if (gameMode === "online") setShowConfirm(true);
    else onQuit();
  };
  ...
  return (
    <div className="flex justify-between items-center mb-2 sm:mb-4 relative px-1 transition-colors duration-300">
      ...
      <div className="flex items-center gap-2 md:gap-3">
        ...
        {/* Dynamic Arena Badge - Neutral background, Accent Icon */}
        <div className="flex items-center gap-0 md:gap-2 px-3 py-2 rounded-full transition-all duration-300 bg-zinc-100 dark:bg-zinc-800">
          <div style={{ color: currentArena?.accent || "#75c32c" }}>
            {getArenaIcon(currentArena?.id)}
          </div>
          <span className="hidden md:block text-[10px] font-black uppercase tracking-widest whitespace-nowrap text-zinc-900 dark:text-zinc-100">
            {currentArena?.name || "Stage 1"}
          </span>
        </div>

        {streak > 0 && (
          <div className="flex items-center gap-2 px-3 py-2 rounded-full bg-orange-500 text-white">
      ...
      <div className="flex items-center gap-2">
        {/* Category Pill - Neutral background, Accent Icon */}
        ...
        {/* Timer Pill: Optimized for high contrast */}
        {gameMode === "online" && (
          <div
